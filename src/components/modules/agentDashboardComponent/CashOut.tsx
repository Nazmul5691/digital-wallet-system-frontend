...
export default function CashOut() {
    ...
    const cashOutTransactions = (data?.data || [])
        ...
        <div className="p-6 bg-gray-50 min-h-screen font-sans">
            ...
            {cashOutTransactions.length === 0 ? (
                <div className="flex items-center justify-center h-48 bg-white rounded-lg shadow-md">
                    <p className="text-gray-500 text-lg">No cash-out transactions found.</p>
                </div>
            ) : (
                            ...
                            <div className="flex flex-col md:flex-row md:justify-between md:items-start space-y-4 md:space-y-0">
                                <div className="flex-1 space-y-2">
                                    ...
                                    <p className="text-md text-gray-600">
                                        <span className="font-medium text-gray-800">User (Sender):</span>{" "}
                                        {tx.senderId?.name}
                                        <span className="text-sm text-gray-500 ml-2">({tx.senderId?.email})</span>
                                    </p>
                                    <div>
                                        <div>
                                            <p className="text-md text-gray-600">
